...
use client::client::{new_conn, get_conn, del_conn};
use std::os::raw::{c_char, c_int, c_uint};
use std::ffi::{CStr, CString};
use client::client::State as client_state;
use client::client::Status as client_status;
use crate::Status::{UnknownStatus, CreatedStatus, RunningStatus, StoppedStatus, DeletedStatus, PauseStatus, PausingStatus};
...
pub extern "C" fn shim_v2_kill(container_id: *const c_char, exec_id: *const c_char, signal: u32, all: bool) -> c_int {
    ...
    get_conn(&r_container_id).and_then(|client| {
        client.kill(&r_container_id, &r_exec_id, signal, all).map(|_| {
...
#[no_mangle]
pub extern "C" fn shim_v2_exec(container_id: *const c_char, exec_id: *const c_char, terminal: bool,
    stdin: *const c_char, stdout: *const c_char, stderr: *const c_char, spec: *const c_char) -> c_int {
    ...
    println!("lib-shim-v2::exec::{}:: [{} {} {} {} {}]",
            r_container_id, r_exec_id, terminal, r_stdin, r_stdout, r_stderr);
    get_conn(&r_container_id).and_then(|client| {
        client.exec(&r_container_id, &r_exec_id, terminal, &r_stdin, &r_stdout, &r_stderr, r_spec).map(|_| {
...
pub extern "C" fn shim_v2_resize_pty(container_id: *const c_char, exec_id: *const c_char,
                                     height: u32, width: u32) -> c_int {
    let (r_container_id, r_exec_id) = (
        to_string(container_id),
        to_string(exec_id)
    );
    println!("lib-shim-v2::resize_pty::{}:: [{}]", r_container_id, r_exec_id);
    get_conn(&r_container_id).and_then(|client| {
        client.resize_pty(&r_container_id, &r_exec_id, height, width).map(|_| {
            println!("lib-shim-v2::resize_pty::{}:: done.", r_container_id);
            0
        })
    }).unwrap_or_else(|e| {
        println!("lib-shim-v2::resize_pty::{}:: failed, {}.", r_container_id, e);
        -1
    })
}
